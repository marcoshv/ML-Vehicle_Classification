...
def main(data_folder, labels, output_data_folder):
    ...
    with open(labels, 'r') as f:
        ...
        #2. Iterate over each row in the CSV
        for row in reader:
            destination_path = output_data_folder+'/'+row["subset"]+'/'+row['class']+'/'
            if not os.path.isdir(destination_path):
                os.makedirs(destination_path)
            #3. Copy the image to the new folder structure
            os.link(data_folder+'/'+row['img_name'], destination_path+row['img_name'])
